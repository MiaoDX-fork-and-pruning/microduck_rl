...
next_action: >-
  Treat adapted `model_750.pt` as the current IsaacLab walking candidate. Its
  ONNX graph and a finite MuJoCo smoke are complete, but cross-backend motion
  is not accepted because direction response changes in MuJoCo. Complete the
  normal sim2real/runtime acceptance before deployment. Keep strict parity's
...
parked_todos: >-
  ONNX export and finite MuJoCo smoke for adapted `model_750.pt` are complete;
  the directional cross-backend acceptance remains pending, and video capture
  needs an image with `omni.replicator`. Preserve all generated battery JSON
