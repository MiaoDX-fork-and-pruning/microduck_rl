...
| Trained-policy battery behavior | mjlab trained-policy acceptance | IsaacLab trained-policy acceptance | BLOCKED | replacement strict run completed 6000 iterations (`model_5999.pt`) with an Isaac-only `clip_actions=1.0` mismatch; its fixed six-case battery fails and is not a valid strict baseline. A bounded post-fix raw-action replay is non-finite, confirming the checkpoint is contaminated by the old boundary; artifact `.cache/isaaclab-assets/velocity_flat_action_boundary_unclipped_model5999_4x50.json`. A fresh post-fix smoke and justified strict run are required; no reward/PPO tuning. | eval |
