...
| Push / foot friction DR | interval pushes + foot material range | IsaacLab event terms | BLOCKED | production PhysX foot-material coefficient write/readback is proven for both ankle bodies in `foot_material_runtime_probe.json`; seeded push reset-subset effect and tangential response fixture remain | task |
