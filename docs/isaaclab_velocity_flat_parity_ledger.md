...
| Trained-policy battery behavior | mjlab trained-policy acceptance | IsaacLab trained-policy acceptance | BLOCKED | corrected strict run completed `4096` envs / `6000` iterations with `clip_actions=None`: `model_5999.pt` (`db0cbd9c...`). Fixed six-case battery is finite with zero resets and tilt <=0.375 rad, but forward and lateral command response fail (actual XY velocity near zero); zero/yaw/turn-left/turn-right pass. The accepted mjlab `model_5999.pt` loaded into the same IsaacLab runner passes all six cases, with forward `(0.118,0.011)` and lateral `(-0.040,0.068)` m/s, so the remaining failure is isolated to the strict checkpoint's learned behavior rather than command/action wiring. A simulator-free audit confirms identical actor/critic tensor schemas, finite normalizer fields, and matching final normalizer counts; a two-way normalizer/weight cross-evaluation shows that replacing only normalizer statistics does not recover forward/lateral response. This makes normalizer drift a material training-distribution observation, but not a runtime semantic mismatch. Artifacts: `.cache/isaaclab-assets/velocity_flat_command_battery_mjlab_policy_p0.json`, `.cache/isaaclab-assets/velocity_flat_checkpoint_audit_p0.json`, `.cache/isaaclab-assets/velocity_flat_normalizer_weight_cross_eval_p0.json`, `.cache/isaaclab-assets/velocity_flat_directional_trace_p0.json`, `.cache/isaaclab-assets/velocity_flat_directional_trace_4x24.json`, `.cache/isaaclab-assets/velocity_flat_directional_trace_resetfix2_4x4.json`, `.cache/isaaclab-assets/velocity_flat_command_battery_resetfix_strict.json`. This remains a trained-policy behavior failure, not a license to tune reward/PPO. | eval |
