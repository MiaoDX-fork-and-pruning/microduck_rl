...
| Contact rewards | air-time, terrain-relative clearance, swing, slip, self collision | `velocity_flat_contact` + ContactSensor/raw PhysX view | BLOCKED | foot-site terms and the direct mjlab count kernel are covered; concrete raw PhysX views now initialize for 4 envs with `(4,2)` trunk and `(4,1)` leg count buffers, but a forced-contact numerical runtime fixture remains | task |
