...
| IMU noise/misalignment | actor noise + random mounting rotation | root-state adapter corruption | BLOCKED | control path and episode-stable state are implemented; runtime task smoke is finite, but sensor source and seeded distribution proof remain pending | task |
...
| Head/body commands | 4D + 6D non-zero commands, 2..5 s resampling | `UniformPoseCommand` manager terms, 13D block | BLOCKED | terms, ranges, staged range curriculum, and 3-term CommandManager initialization are runtime-smoked; resampling distribution proof remains pending | task |
...
| Contact rewards | air-time, terrain-relative clearance, swing, slip, self collision | `velocity_flat_contact` + ContactSensor | BLOCKED | foot-site terms and the direct mjlab count kernel are covered; the filtered self-contact view passes with one environment but fails to initialize with four (`expected 8, found 4`), so a multi-env-safe source and runtime proof remain | task |
