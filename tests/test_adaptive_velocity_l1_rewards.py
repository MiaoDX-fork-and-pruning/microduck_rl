...
def _yaw_planar_tracking(env):
    return mdp.track_linear_velocity_yaw_averaged(
        env, std=0.12, command_name="twist", tau_s=FEEDBACK_TRACKING_TAU_S,
    )


def test_yaw_gaussian_retains_canonical_idle_and_translation_response():
    from mjlab.tasks.velocity.mdp.rewards import track_linear_velocity

    env = _env([[0, 0, 0], [0.12, 0, 0], [0, -0.12, 0.05]])
    for step in range(1, 101):
        env.common_step_counter = step
        env.episode_length_buf[:] = step
        env.scene["robot"].data.root_link_lin_vel_b[:] = torch.tensor(
            [0.05, 0.15 * (-1) ** step, 0.03], dtype=torch.float64,
        )
        torch.testing.assert_close(
            _yaw_planar_tracking(env),
            track_linear_velocity(env, std=0.12, command_name="twist"),
            rtol=0, atol=0,
        )


@pytest.mark.parametrize("yaw", [-0.8, 0.8])
def test_yaw_gaussian_allows_alternating_sway_but_prices_drift_and_bounce(yaw):
    env = _env([[0, 0, yaw], [0, 0, yaw], [0.08, 0, yaw], [0, 0, yaw]])
    data = env.scene["robot"].data
    late_rewards = []
    for step in range(1, 301):
        env.common_step_counter = step
        env.episode_length_buf[:] = step
        ripple = 0.17 * torch.sin(torch.tensor(2 * torch.pi * 2 * step * env.step_dt))
        data.root_link_lin_vel_b[:, 1] = ripple
        data.root_link_lin_vel_b[1, 1] += 0.12  # Persistent drift at yaw.
        data.root_link_lin_vel_b[2, 0] = 0.08  # Moving turn tracks translation.
        data.root_link_lin_vel_b[3, 2] = 0.12 * (-1) ** step
        reward = _yaw_planar_tracking(env)
        if step > 100:
            late_rewards.append(reward)
    mean = torch.stack(late_rewards).mean(dim=0)
    assert mean[0] > 0.95
    assert mean[1] < 0.40  # Averaging must not erase sustained planar error.
    torch.testing.assert_close(mean[2], mean[0])
    torch.testing.assert_close(mean[3], mean[0] * torch.exp(torch.tensor(-1.0)))


def test_yaw_gaussian_resets_average_and_shares_one_update_with_l1():
    env = _env([[0, 0, 0.8]] * 3, linear=[[0.12, 0, 0]] * 3)
    _yaw_planar_tracking(env)
    env.common_step_counter = 1
    env.episode_length_buf[:] = 20
    env.episode_length_buf[0] = 1
    env.command_manager.get_command("twist")[1, 0] = 0.08
    env.scene["robot"].data.root_link_lin_vel_b.zero_()
    reward = _yaw_planar_tracking(env)
    assert reward[0] == 1.0
    assert reward[1].item() == pytest.approx(torch.exp(torch.tensor(-(0.08 / 0.12) ** 2)).item())
    assert reward[2] < 0.41
    _averaged_linear(env)
    _averaged_yaw(env)
    torch.testing.assert_close(_yaw_planar_tracking(env), reward, rtol=0, atol=0)


def test_yaw_gaussian_is_limited_to_lateral_drive_recipe():
    from mjlab.tasks.velocity.mdp.rewards import track_linear_velocity
    ...
    for axis, diagnostic, feedback, _ in ADAPTIVE_RECIPES:
        ...
        expected = mdp.track_linear_velocity_yaw_averaged if diagnostic == "lateral_drive" else track_linear_velocity
        assert cfg.rewards["track_linear_velocity"].func is expected
    assert canonical.rewards["track_linear_velocity"].func is track_linear_velocity
    assert make_microduck_velocity_env_cfg().rewards == canonical.rewards
